...
def cumulative_poisson(x):
    return poisson.cdf(x, POISSON_P)
...
def cumulative_uniform(x):
    return uniform.cdf(x, UNIFORM_L, 2 * UNIFORM_R)
...
def generate_uniform(x):
    return np.random.uniform(UNIFORM_L, UNIFORM_R, x)


def generate_poisson(x):
    return np.random.poisson(POISSON_P, x)
...
def draw_kernel(sample, func, sector, h):
    if sector == 3:
        plt.title(
            'n = ' + str(len(sample)) + '.Kernel density estimation, h = 0.3, 0.6, 1.2 Distribution: ' + func)
    plt.subplot(130 + sector)
    if func == 'poisson':
        xx = np.linspace(6, 14, 100)
    else:
